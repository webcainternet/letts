...
WHERE p.post_type = 'profissional' AND pm.meta_key = 'profissao' AND pm.meta_value = '".$_POST["profissao"]."' AND pm2.meta_key = 'basicaemail') ) AS t1 order by rand() limit 1");
	   ...
	   //Randomiza Video ID
	   $resultIDNull = mysql_query("SELECT id FROM (SELECT * FROM wp_posts WHERE post_type = 'video' AND post_status = 'publish' ORDER BY post_date_gmt DESC limit 5) AS t1 order by rand() limit 1");
  } }
