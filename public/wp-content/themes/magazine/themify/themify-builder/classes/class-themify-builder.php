...
if ( ! class_exists( 'Themify_Builder' ) ) {
	...
	class Themify_Builder {
		...
		function init() {
			...
			// Switch to frontend
			add_action( 'wp_ajax_tfb_switch_frontend', array( &$this, 'switch_frontend' ), 10 );
		...
		function load_front_js_css() {
			...
			// module scripts
			wp_enqueue_script( 'themify-easy-pie-chart', THEMIFY_BUILDER_URI . '/js/jquery.easy-pie-chart.js', array( 'jquery' ), THEMIFY_VERSION, true );
		 ...
		 * @param int $post_id
		 */
		function switch_frontend( $data = null ) {
			check_ajax_referer( 'tfb_load_nonce', 'nonce' );
			global $wpdb;
